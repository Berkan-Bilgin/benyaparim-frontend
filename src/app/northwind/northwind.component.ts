...
export class NorthwindComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.registerForm = this.formBuilder.group({
      name: ['', Validators.required], // Ürün adı için zorunlu alan validator'ü
      price: [null, [Validators.required, Validators.min(1)]], // Fiyat için zorunlu alan ve minimum değer validator'leri
    });
  }
  ...
  onSubmit(): void {
    if (this.registerForm.valid) {
      ...
      this.northwindService.addProduct(this.registerForm.value).subscribe({
        next: (product) => {
